...
exports.getDatabyMsgSessionId = function (req, res, next) {
  ...
  _chat2.default.find({ MsgSessionId: { $in: [req.params.user_mobile + '_' + req.params.sender_mobile, req.params.sender_mobile + '_' + req.params.user_mobile] } }).sort({ msg_timestamp: 1 }).select('-_id').exec(function (err, chat) {
...
exports.create = function (req, res, next) {
  ...
  _chat2.default.create(req.body, function (err, chat) {
    if (err) return res.status(201).json({ success: false, message: 'Something went worng!' });
